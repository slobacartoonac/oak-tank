import { colide, distanceZ } from "./globalFunctions";
import { Truper } from "./truper";

export function Enemy(team) {
    this.team = team;
    this.shield = 1;
    this.oldControl = { a: 0, w: 0, s: 0, d: 0, e: 0, f: 0 };
    this.oldStable = { x: -1, y: -1, z: -1 };
    this.vehicleOldPosition = { x: -1, y: -1, z: -1 };
}
...
Enemy.prototype.getInstructions = function (vehicle, physics) {
    ...
    if (((this.oldStable.x != vehicle.stable.x || this.oldStable.z != vehicle.stable.z) && colide(vehicle.stable, vehicle.model.position, 5)) ||
        ...
        this.oldStable.z = vehicle.stable.z;
        ...
        for (var i = 0; i < 4; i++) {
            ...
            var coliders = neer1.coliders.concat(neer2.coliders).concat(neer3.coliders);
        ...
        cando.coliders = cando.coliders.filter(function (el) {
            return el != vehicle &&
                !((el instanceof Truper) && el.driver.team != vehicle.driver.team);
        });
        if (cando.map < 2 && cando.coliders.length == 0) {
            //vehicle.move(x,y);
            ret.a = x < 0 ? 1 : 0;
            ret.d = x > 0 ? 1 : 0;
            ret.s = y < 0 ? 1 : 0;
    ...
    if (vehicle.shootTimeOut < 0) {
        ...
        if (!ret.f) vehicle.shootTimeOut = 0.5;
    }
    if (vehicle.helth < 20) ret.e = 1;
    this.oldControl = ret;

    this.vehicleOldPosition.x = vehicle.model.position.x;
    this.vehicleOldPosition.z = vehicle.model.position.z;
    this.vehicleOldPosition.y = vehicle.model.rotation;

    return ret;
}
var masterPlayer = null;
export function Player(team) {
