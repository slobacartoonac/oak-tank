function Projectile(shooter,model,trail,speed,range,dmg)
{
    this.typeNum=0;
    this.model=model;
    this.model1=trail;
...
Projectile.prototype.update=function(time,physics)
{
    ...
    this.lightsize=15*Math.sin(this.range)*5;
    ...
    for(var i=0;i<col.coliders.length;i++)
    {
        ...
        if(hit!=this.shooter){
            this.valid=false;
            hit.demage(this.dmg,distanceZ(hit.model.position,this.model.position),physics);
        }
    }
}

function Bulet(shooter,trail,range,dmg)
{
    this.typeNum=0;
    this.model=trail;
    this.range=range;
    ...
    this.light=[2.,1.5,1.5];
    this.lightsize=20;
    this.rotation=this.shooter.model.rotation;
...
Bulet.prototype.update=function(time,physics)
{
    ...
    while(!this.calculated&&this.range>0){
        ...
        for(var i=0;i<col.coliders.length;i++)
        {
            ...
            if(hit!=this.shooter){
                this.calculated=true;
                hit.demage(this.dmg,distanceZ(hit.model.position,{x:tx,y:this.model.position.y,z:ty}),physics);
            }
        }
        tx+=addx;
        ty+=addy;
...
{
    this.model.position.x=this.owner.model.position.x;
    this.model.position.z=this.owner.model.position.z;
    this.model.position.y=this.owner.model.position.y+this.plusY;
